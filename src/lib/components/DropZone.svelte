  ...
  <div class="space-y-4">
    ...
    <div>
      <p class="text-lg font-medium theme-heading">
        {isDragOver ? 'Dateien hier ablegen' : 'Markdown-Dateien hierher ziehen'}
      </p>
      <p class="text-sm theme-text mt-1">
        oder <button class="theme-accent hover:underline font-medium focus-ring rounded">
          Dateien durchsuchen
        </button>
      </p>
      <p class="text-xs theme-text mt-2 opacity-75">
        Unterstützt .md Dateien bis 10MB
      </p>
    </div>
  </div>
...
<p id="file-input-description" class="sr-only">
  Wähle Markdown-Dateien zum Hochladen. Mehrere Dateien werden unterstützt.
</p>